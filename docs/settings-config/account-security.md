---
sidebar_position: 1
title: Account & Security
---

# Account and Security Settings

The **Account** and **Security** tabs allow all users to manage their personal profile information and maintain account protection.

:::info ACCESS
This section is visible and editable by **all users**, regardless of their role or scope.
:::

---

## 1. Account Tab (Personal Details)

The Account tab displays your personal information and current organisational context.

### A. Viewing Account Details

This section allows you to quickly verify your credentials and current status.

* **Name:** Your full name as recorded in the system.
* **Email Address:** The primary email address associated with your Vela account.
* **Current Organisation:** The name of the organisation you are currently active in.

### B. Switching Organisations

If you are a member of multiple organisations, you can easily change your active context.

1.  Navigate to **Settings** → **Organisations** → **My Orgs**.
2.  Locate the organisation you wish to access in the list.
3.  Click the **Switch** button next to the organisation's name to make it your current active organisation.

---

## 2. Security Tab (Change Password)

The Security tab is dedicated to protecting your account by allowing you to update your password.

### Step 1: Access the Change Password Section

1.  Navigate to **Settings** → **Security**.
2.  Locate the **Change your password** form.

### Step 2: Input Your Details

* **Current Password:** Enter your existing password to authorise the change.
* **New Password:** Enter your desired new password.
* **Verify Password:** Re-enter the new password exactly as typed to confirm it.

### Password Requirements

To ensure strong security, your new password must meet the following criteria:

* At least **8 characters**.
* At least **one letter** (a–z, A–Z).
* At least **one number** (0–9).
* At least **one special character** (e.g., `@`, `#`, `!`).

### Step 3: Save Changes

1.  Once all fields are complete and the new password meets the requirements, click the **Save** button.
2.  You will be prompted to log in again with your new credentials.