...
| **Historical Search** | On / Off | Whether the search also runs against interactions already in Vela |
| **Link to Search** | Off, or a main search | The search is only checked on interactions a chosen main search has already matched. The main search must be in the same scope |
...
All of these can be changed after creation by editing the search.
